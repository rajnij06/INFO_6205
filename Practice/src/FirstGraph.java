...
class Graph<T>
{
    private Map<T, List<T>> map = new HashMap<>(); //creating an object of the Map class that stores the edges of the graph
    public void addNewVertex(T s) //the method adds a new vertex to the graph
    {
        map.put(s, new LinkedList<T>());
    }
    public void addNewEdge(T source, T destination, boolean bidirectional)//the method adds an edge between source and destination
    {
    ...
    public void countEdges(boolean bidirection) //variable to store number of edges
    {
        int count = 0;
        for (T v : map.keySet())
        {
    ...
    public void containsVertex(T s)     //checks a graph has vertex or not
    {
